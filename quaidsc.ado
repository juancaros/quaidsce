...
cap program drop quaidsc
program define quaidsc, eclass
	version 14.0
	syntax varlist 
	local nargs `varlist'
	args `varlist'
...
nlsur quaidsc @ `nargs2' , nls neq(`J1') parameters(`param') 
...
matrix e[`i',`j']=mu`i'`j'*cdf`i'mean/wu`i'mean-`delta'+pdf`i'mean*`d`i''
}
}
